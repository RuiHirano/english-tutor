...
- **間隔反復**：忘れかけたタイミングでの復習
- **深い処理**（Craik & Lockhart）：同じ教材を多角的に処理
...
│   └── skills/
│       ├── material-new/SKILL.md      # 新教材生成（未解決ミスを織り込む）
...
│   │   ├── material.py                #   教材 + vocabulary_items を一括 INSERT
...
│       └── material_detail.py         # 教材ごとの vocab・session・Q&A
│
...
| `python -m english_tutor.flow.material` | 教材＋vocabulary_items 一括 INSERT（stdin JSON） |
