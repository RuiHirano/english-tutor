...
description: 工程3 — 多聴。コア教材で全体把握 → 内容把握問題。別教材・過去教材の多聴も実施。
---
...
- **コア多聴**：active なコア教材に対して `phase = listening` で session を開く
- **extensive 多聴**：別教材／過去教材に対して `phase = extensive_listening` で session を開く（問題なし）
...
1. 古い教材（`mastery_level >= 1` かつ `last_appeared_at` が 2 週間以上前）または未学習の extensive 教材から 1〜2 本選ぶ
